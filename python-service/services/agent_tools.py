...
@tool
async def search_course_materials(query: str, week_slug: str | None = None) -> str:
...
@tool
async def get_vocabulary_terms(lesson_slug: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=TOOL_TIMEOUT) as client:
            ...
            if response.status_code != 200:
                return f"Could not fetch vocabulary for lesson '{lesson_slug}'. The lesson slug may be incorrect — use get_week_lessons first to discover valid slugs."
        ...
        return f"Request timed out fetching vocabulary for '{lesson_slug}'. The server may be busy — try again."
    except httpx.ConnectError:
        return f"Could not connect to the API server. The service may be down."
    except Exception as e:
...
@tool
async def get_vocabulary_stats(lesson_slug: str, session_id: str = "anonymous") -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=TOOL_TIMEOUT) as client:
            ...
            if response.status_code != 200:
                return f"Could not fetch stats for lesson '{lesson_slug}'. The lesson slug may be incorrect — use get_week_lessons first to discover valid slugs."
        ...
        return f"Request timed out fetching stats for '{lesson_slug}'. The server may be busy — try again."
    except httpx.ConnectError:
        return f"Could not connect to the API server. The service may be down."
    except Exception as e:
...
@tool
async def generate_practice_question(week_slug: str) -> str:
...
@tool
async def get_lesson_content(lesson_slug: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=TOOL_TIMEOUT) as client:
            ...
            if response.status_code != 200:
                return f"Could not fetch lesson '{lesson_slug}'. The lesson slug may be incorrect — use get_week_lessons first to discover valid slugs."
        ...
        return f"Request timed out fetching lesson '{lesson_slug}'. The server may be busy — try again."
    except httpx.ConnectError:
        return f"Could not connect to the API server. The service may be down."
    except Exception as e:
...
@tool
async def get_week_lessons(week_slug: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=TOOL_TIMEOUT) as client:
            ...
            if response.status_code != 200:
                return f"Could not fetch week '{week_slug}'. The week slug may be incorrect."
        ...
        return f"Request timed out fetching week '{week_slug}'. The server may be busy — try again."
    except httpx.ConnectError:
        return f"Could not connect to the API server. The service may be down."
    except Exception as e:
